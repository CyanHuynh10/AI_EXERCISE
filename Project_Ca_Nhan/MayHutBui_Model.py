...
class MayHutBui:
    ...
    def setup_ui(self):
        ...
        self.algo_combo = ttk.Combobox(algo_frame, font=("Arial", 11), state="readonly", 
                                       values=["BFS", "DFS", "IDS", "UCS", "A*", "IDA*", 
                                               ...
                                               "Quan sát một phần", "Không quan sát được"])
    ...
    def run_algorithm(self):
        ...
        elif algo == "Quan sát một phần":
            final_path = self.partially_observable_search(start_state)
        elif algo == "Không quan sát được":
            final_path = self.unobservable_search(start_state)
    ...
    def partially_observable_search(self, start_state):
        current_state = start_state
        path = [current_state]
        visited = set()
        stack = []
        ...
        while len(dirty) > 0:
            ...
            for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if 0 <= nr < self.grid_size and 0 <= nc < self.grid_size and (nr, nc) not in visited:
                    ...
                    dirty = tuple(d for d in dirty if d != (r, c))
                    current_state = (r, c, dirty)
                    path.append(current_state)
                    moved = True
                    break
    ...
    def unobservable_search(self, start_state):
        path = []
        r, c = start_state[0], start_state[1]
        dirty = start_state[2]
        current_state = (r, c, dirty)
        path.append(current_state)
        ...
        for row in range(self.grid_size):
            if row % 2 == 0:
                ...
                cols = range(self.grid_size - 1, -1, -1)
